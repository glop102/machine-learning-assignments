#source : https://gist.github.com/jonholifield/617202041970c23c0f5cc5c664883343
...
if __name__ == '__main__':
    ...
    Q = tf.reduce_sum(tf.matmul(action_values, tf.transpose(actions)), reduction_indices=1) 
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            for step in range(max_steps):
                ...
                if done:
                    samples = [ D[i] for i in sorted(random.sample(range(len(D)), sample_size)) ]
                    #print samples
                    y_ = []
                    states_samples = []
                    next_states_samples = []
