...
def residualSection(node,features1,features2):
	...
	return tf.nn.l2_normalize(n3,0) + node

def denseUnit(node,shape):
	w = tf.Variable(tf.truncated_normal(shape, stddev=0.1))
	b = tf.Variable(tf.constant(0.1, shape=[shape[-1]]))
	return tf.nn.relu(tf.matmul(node,w)+b)

l1 = tf.nn.relu(convolutionUnit(input_node,[3,3,3,20]))
l1 = tf.nn.relu(convolutionUnit(l1,[3,3,20,50]))
for _ in range(2):
	l1 = residualSection(l1,30,50)
l2 = tf.nn.max_pool( convolutionUnit(l1,[3,3,50,125]) , [1,2,2,1],[1,2,2,1],"SAME") #16x16
for _ in range(2):
	l2 = residualSection(l2,75,125)
l3 = tf.nn.max_pool( convolutionUnit(l2,[3,3,125,250]) , [1,2,2,1],[1,2,2,1],"SAME") #8x8
for _ in range(2):
	l3 = residualSection(l3,125,250)
l4 = convolutionUnit(l3,[3,3,250,600])
for _ in range(2):
	l4 = residualSection(l4,450,600)
l5 = tf.nn.max_pool(l4,[1,2,2,1],[1,2,2,1],"SAME") #4x4
l5 = tf.reshape(l5,[-1,4*4*600])
l5 = tf.nn.dropout(l5,dropout_keeprate)

l6 = denseUnit(l5,[4*4*600,2048])
